...
def generate_pkcsv15_padded_hash(in_hash):
    ...
    pkcsv15_padded_hash = struct.pack("!BB{}sB{}s{}s".format(padding_len, len(oid), len(hash_val)), 0, 1, '\xFF'*padding_len, 0, oid, hash_val)

    if (ENABLE_DEBUG == True):
        print("PKCSv15 padded hash: {} \n".format(binascii.hexlify(pkcsv15_padded_hash[0:])))
...
def ct_compute_signature(decoded_challenge):
    #compute sha-512 on entire challenge
    challenge_hash = hashlib.sha512(decoded_challenge).hexdigest()

    #generate pkcsv15 padded hash
    padded_hash = generate_pkcsv15_padded_hash(challenge_hash)

    #read the OpenSSL generated key file
    with open(CT_DEMO_SIGNING_PRIKEY_FILE, 'r') as f:
        ct_key = RSA.importKey(f.read())

    #sign the challenge using the imported key
    signature = ct_key.decrypt(padded_hash)
...
class CT_Challenge:
    ...
    def ct_verify_challenge_info(self, decoded_challenge):
        ...
        #verify function codes supported in each version
            ...
            try:
                ...
                #platform nonce
                    ...
                    if (l != CT_NONCE_LENGTH):
                        ...
                        return 0
                    self.nonce = binascii.hexlify(decoded_challenge[i:i+l])
                ...
                #sto nonce
                    ...
                    if (l != CT_RANDOM_NUMBER_LENGTH):
                        ...
                        return 0
                    self.random_number = binascii.hexlify(decoded_challenge[i:i+l])
                ...
                #product name
                    ...
                    self.prod_name = struct.unpack('!{}s'.format(l), decoded_challenge[i:i+l])[0]
                ...
                #key name
                    ...
                    self.key_name = struct.unpack('!{}s'.format(l), decoded_challenge[i:i+l])[0]
                ...
                #pid
                    ...
                    if ((l == 0 ) or (l > CT_MAX_PID_LENGTH)):
                        ...
                        return 0
                    self.pid_length = l
                    self.pid = struct.unpack('!{}s'.format(l), decoded_challenge[i:i+l])[0]
                ...
                #sn
                    ...
                    if ((l == 0 ) or (l > CT_MAX_SN_LENGTH)):
                        ...
                        return 0
                    self.sn_length = l
                    self.sn = struct.unpack('!{}s'.format(l), decoded_challenge[i:i+l])[0]
...
class CT_Response:
    ...
    def ct_compute_response(self, args, version, function_code, sub_function_code, pid, sn, decoded_challenge):
        ...
        enc_signature = ''
        enc_signature = base64.b64encode(self.signature)

        #split encoded signature accross multiple lines, with max length of each line = 64
        if (len(enc_signature) % 64):
            loop = (len(enc_signature) / 64) + 1
        else:
            loop = (len(enc_signature) / 64)
